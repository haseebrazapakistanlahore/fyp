...
class Product extends Model
{
    ...
    protected $appends = ['favorite_product_id','in_wishlist'];

    public function getFavoriteProductIdAttribute(){
        return Product::where('id', $this->id)->first()->id;
    }
    
    public function getInWishlistAttribute(){
        $request = request();
        if(isset($this->favorite_product_id)){
            if(isset($request->wishlist_consumer_id)){
                $product = Favourite::whereNotNull('consumer_id')->where('consumer_id', $request->wishlist_consumer_id)->where('product_id', $this->favorite_product_id)->first();
                if($product == null){
                    return 0;
                }else{
                    return 1;
                }
            }else if(isset($request->wishlist_professional_id)){
                 $product = Favourite::whereNotNull('professional_id')->where('professional_id', $request->wishlist_professional_id)->where('product_id', $this->favorite_product_id)->first();
                if($product == null){
                    return 0;
                }else{
                    return 1;
